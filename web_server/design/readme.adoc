...
- `RequestData` contains shared memory for future stages.
- Each `RequestData` includes:
...
- Decomposes each received request into smaller, independent processing units called `RequestUnit`.
- Each `RequestUnit` includes:
- A pointer to a shared `RequestData`. 
...
- Takes individual `RequestUnit`s and performs the required computation.
- Stores the results in the corresponding result vector of the unit.
- Send the processed `RequestUnit` to the `RepliesAssembler`.
...
- Gathers all `RequestUnit`s belonging to the same original request.
- Send completed `RequestData` object to the ClientResponder.

=== ClientRespond (Consumer)

**Role**: Builds the response and sends it to the client.

**Functionality**:

- Build response with the `RequestData` object and sends it back to the client.
- Delete de `RequestData` object to free memory.
